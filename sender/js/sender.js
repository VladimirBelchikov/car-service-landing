...
function collect(form, delay) {
    if (!loading) {
        ...
        if (typeof ymaps !== 'undefined' && typeof ymaps.geolocation !== 'undefined') {
            cityYmaps = ymaps.geolocation.city;
        }

        let saveData = $('[name=save-data]', form).val()
        var data = {
            city: cityYmaps,
        ...
        $.ajax('/sender/send.php', {
            ...
            success: function (response) {
                loading = false;
                if (!delay) {
                    !saveData ? yaMetrikaReachGoal('supercel') : null;
                    saveData ? sessionStorage.removeItem('formData') :  sessionStorage.setItem('formData', JSON.stringify(data));
